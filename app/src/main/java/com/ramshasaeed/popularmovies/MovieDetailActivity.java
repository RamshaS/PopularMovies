            ...
            imgFavourite.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (!movie.getFavourite()) {
                        ...
                        final LiveData<Favourite> fav = mDb.favouriteDao().loadTaskById(movie.getId());
                        fav.observe(MovieDetailActivity.this, new Observer<Favourite>() {
                            @Override
                            public void onChanged(@Nullable Favourite favourite1) {
                            ...
                            AppExecutors.getInstance().diskIO().execute(new Runnable() {
                                @Override
                                public void run() {
                                    Favourite fav = new Favourite(movie.getId(),movie.getPosterUrl(),movie.getOriginal_title(),movie.getOverview(),movie.getVote_average(),movie.getRelease_date(),movie.getBackdrop_path());
                                    if (favourite == null){
                                        mDb.favouriteDao().insertFavourite(fav);
                                    }else{
                                        mDb.favouriteDao().updateFavourite(fav);
                                    }
                                }
                            });
    ...
    AppExecutors.getInstance().diskIO().execute(new Runnable() {
        ...
        public void run() {
            Favourite fav = new Favourite(movie.getId(),movie.getPosterUrl(),movie.getOriginal_title(),movie.getOverview(),movie.getVote_average(),movie.getRelease_date(),movie.getBackdrop_path());
    mDb.favouriteDao().deleteFavMovie(fav);
        }
    });
